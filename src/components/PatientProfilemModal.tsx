...
//============= que patient ============
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm px-2 py-8 overflow-y-auto">
      ...
      <ConfirmationPage
      isOpen={!!newPatient && Object.keys(newPatient).length > 0}
      onClose={onClose}
      name={(newPatient as any)?.full_name || ""}
      address={(newPatient as any)?.address || ""}
      patientId={(newPatient as any)?.patient_id || ""}
      phone={(newPatient as any)?.phone || ""}
      />
