    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-100">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-8 py-6 space-y-6">
        ...
        <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-sm border-b border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between px-4 py-4 rounded-t-xl shadow-sm">
          ...
          <div className="mt-3 md:mt-0 flex w-full md:w-auto gap-2">
            <input
              ...
              className="w-full md:w-64 px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 outline-none"
            />
            <button
        ...
        <div className="grid grid-cols-3 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          <StatCard icon={MdOutlineInventory} title="Total Orders" value={pharmacyData.length} />
          <StatCard icon={BiTask} title="Pending Orders" value={pharmacyData.filter(order => order.status === "pending").length} />
          <StatCard icon={BiTask} title="Completed" value={pharmacyData.filter(order => order.status === "completed").length} />
          <StatCard icon={BiTaskX} title="Cancelled" value={pharmacyData.filter(order => order.status === "cancelled").length} />
        </div>
        ...
        <section className="bg-white rounded-xl shadow border border-slate-200 p-4 md:p-6 overflow-x-auto">
          ...
          {!loading && pharmacyData.length === 0 && (
            <div className="text-gray-500 py-6 text-center">No orders found.</div>
          )}
          ...
          {!loading && pharmacyData.length > 0 && (
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody>
                ...
                {pharmacyData.filter(order => order.status === "pending").map((order) => (
                  <tr className="hover:bg-blue-50 transition border-b border-gray-200" key={order.id}>
                    ...
                    <td className="px-4 py-3 flex flex-col md:flex-row gap-2 border-r">
                      <button
                        className="bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded text-white text-xs font-medium"
                      ...
                      <button
                        className="bg-green-600 hover:bg-green-500 px-3 py-1.5 rounded text-white text-xs font-medium"
